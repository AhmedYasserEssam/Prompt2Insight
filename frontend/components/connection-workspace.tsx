...
export function ConnectionWorkspace() {
  ...
  const [pending, setPending] = useState<"test" | "save" | "select" | "refresh" | null>(null);
  ...
  useEffect(() => {
    void listConnectionProfiles().then((loaded) => {
      ...
      if (savedProfile) void select(savedProfile);
  ...
  async function select(profile: ConnectionProfile) { setPending("select"); setMessage(null); try { const progress = await selectConnection(profile.id); setActiveProfileId(profile.id); window.localStorage.setItem(SELECTED_CONNECTION_KEY, profile.id); if (progress.conversation_id) setConversationId(progress.conversation_id); else setMessage(t.configured); } catch (error) { setMessage(error instanceof Error ? error.message : t.failed); } finally { setPending(null); } }
  ...
  return <section className="stack" dir={locale === "ar" ? "rtl" : "ltr"} lang={locale}>
    <div className="row workspace-header"><h2>{conversationId ? "Prompt2Insight" : t.settings}</h2><div className="row"><button type="button" className="secondary" onClick={() => setLocale(locale === "en" ? "ar" : "en")}>{locale === "en" ? "العربية" : "English"}</button>{activeProfile ? <label className="connection-selector">{t.active}<select value={activeProfileId ?? ""} onChange={(event) => { const profile = profiles.find((item) => item.id === event.target.value); if (profile) void select(profile); }}><option value={activeProfile.id}>{profileLabel(activeProfile, t.ready)}</option>{profiles.filter((profile) => profile.id !== activeProfile.id && profile.state === "ready").map((profile) => <option key={profile.id} value={profile.id}>{profileLabel(profile, t.ready)}</option>)}</select></label> : null}</div></div>
    {conversationId ? <AnalyticsChat conversationId={conversationId} /> : <>
