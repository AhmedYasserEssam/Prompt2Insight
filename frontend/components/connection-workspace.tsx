...
export function ConnectionWorkspace() {
  ...
  return <section className="stack" dir={locale === "ar" ? "rtl" : "ltr"} lang={locale}>
    ...
    {profiles.length > 0 ? <div className="card stack"><h3>{t.settings}</h3>{profiles.map((profile) => <div className="stack" key={profile.id}><div className="row"><span dir="ltr">{profile.name} · {profile.dialect} · {profile.host}/{profile.database_name} · {profile.state}</span>{profile.state === "ready" ? <><button disabled={pending !== null} onClick={() => void select(profile)}>{t.select}</button><button type="button" className="secondary" disabled={pending !== null} onClick={() => setCatalogProfileId(profile.id)}>{t.editCatalog}</button></> : <button type="button" disabled={pending !== null} onClick={() => setCatalogProfileId(profile.id)}>{profile.state === "stale" ? t.revalidateCatalog : t.configureCatalog}</button>}<button type="button" className="secondary" disabled={pending !== null} onClick={() => void refresh(profile)}>{pending === "refresh" ? t.refreshing : t.refreshSchema}</button></div><small>{t.refreshHelp}</small>{profile.state === "stale" ? <span className="error">{t.stale}</span> : null}</div>)}<button type="button" className="secondary" onClick={() => setShowForm(true)}>{t.open}</button></div> : null}
